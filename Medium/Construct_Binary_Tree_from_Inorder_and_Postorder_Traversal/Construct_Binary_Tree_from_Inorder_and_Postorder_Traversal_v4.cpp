...
public:
    ...
    TreeNode* help(vector<int>&inorder,vector<int>&postorder,int &post_index,int inS,int inE){
        ...
        //first right is constructed because in postorder array we are going from backward direction and the array is of type LRN so first we encounter the right part that's why right.
