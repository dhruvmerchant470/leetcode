...
class Solution {
public:
    unordered_map<int,int>nextGreater(vector<int>&vec){
        stack<int>s;
        s.push(-1);
    ...
    TreeNode* bst(int &pre_index,int s_index,int e_index,vector<int>&preorder,unordered_map<int,int>&mp){
    ...
    TreeNode* bstFromPreorder(vector<int>& preorder) {
        unordered_map<int,int>mp=nextGreater(preorder);
